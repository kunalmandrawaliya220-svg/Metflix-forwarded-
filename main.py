...
# ====================================
# FIXED CONFIG (ONLY THESE TWO CHANNELS)
# ====================================
API_ID = 26361463
...
SOURCE = -1002657960619      # FINAL source channel ID
...
async def forward_old():
    ...
    try:
        async for msg in client.get_chat_history(SOURCE, limit=0):
            ...
            try:
                await client.copy_message(DEST, SOURCE, msg.id)
                print("OLD →", msg.id)
...
async def start_bot():
    ...
    # validate SOURCE
    try:
        src = await client.get_chat(SOURCE)
    ...
    # validate DEST
    try:
        dest = await client.get_chat(DEST)
