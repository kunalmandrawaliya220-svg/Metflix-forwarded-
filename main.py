...
async def forward_old_messages(app2):
    ...
    async for msg in app2.get_chat_history(config.SOURCE):
        try:
            await msg.forward(config.DEST)
...
async def start_pyrogram():
    ...
    @app2.on_message(filters.chat(config.SOURCE))
    async def forward_handler(client, message):
        try:
            await message.forward(config.DEST)
